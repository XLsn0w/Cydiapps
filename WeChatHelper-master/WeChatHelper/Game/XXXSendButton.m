...
@implementation XXXSendButton

+ (instancetype)sharedInstance {
    
    static XXXSendButton *instance = nil;
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        
        instance = [[XXXSendButton alloc] init];
    });
...
- (instancetype)init {
    ...
    if(self = [super init]) {
        ...
        self.frame = CGRectMake(x, LTSHeight/2 - 88, 45, 45);
...
- (void)gotoFuck {
    [XXXAllButtonsView sharedInstance].hidden = ![XXXAllButtonsView sharedInstance].hidden;
